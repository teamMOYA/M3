...
public class Model {
	...
	private void initConnexio() {
		/* Crear emf i em */
		emf = Persistence.createEntityManagerFactory(examens);
	...
	public ObservableList<Examen> getExamens() {
		...
		initConnexio();
		//TODO borrar toda la lista??
		TypedQuery<Examen> query = em.createQuery("SELECT e FROM Examens e", Examen.class);
		List<Examen> listExamens = query.getResultList();

		for(Examen examen : listExamens){
			this.examens.add(examen);
		}

		this.examens = FXCollections.observableArrayList(listExamens);
	...
	public ObservableList<Estudiant> getEstudiants() {
		...
		initConnexio();
		//TODO borrar toda la lista??
		TypedQuery<Estudiant> query = em.createQuery("SELECT e FROM Estudiants e", Estudiant.class);
		List<Estudiant> listEstudiants = query.getResultList();
		
		for(Estudiant estudiant : listEstudiants){
	...
	public ObservableList<UnitatFormativa> getUnitats() {
		...
		TypedQuery<UnitatFormativa> query = em.createQuery("SELECT e FROM Estudiants e", UnitatFormativa.class);
		List<UnitatFormativa> listUnitats = query.getResultList();

		this.unitats = FXCollections.observableArrayList(listUnitats);
		
		for(UnitatFormativa unitat : listUnitats){
			this.unitats.add(unitat);
		}
